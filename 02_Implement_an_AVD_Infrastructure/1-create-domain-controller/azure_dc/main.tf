...
locals {
  ...
  configure_ad_command = "Install-ADDSForest -CreateDnsDelegation:$false -DomainMode Windows2016 -DomainName ${var.active_directory_domain} -DomainNetbiosName ${var.active_directory_netbios_name} -ForestMode Windows2016 -InstallDns:$true -SafeModeAdministratorPassword $password -Force:$true"
